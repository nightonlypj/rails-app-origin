  ...
  describe 'POST #create' do
    ...
    shared_context '招待依頼・完了日時変更' do |completed_at|
      before do
        if requested_at.present? || completed_at.present?
          login_user.invitation_requested_at = requested_at
          login_user.invitation_completed_at = completed_at
          login_user.save!
        end
      end
    end
    ...
    shared_examples_for 'ToError' do
      it '成功ステータス' do # Tips: 再入力
        post create_user_session_path, params: { user: attributes }, headers: headers
        expect(response).to be_successful
      end
    end
    ...
    shared_examples_for '[未ログイン][ある][ない][*]ベースドメイン' do
      let!(:headers) { BASE_HEADER }
    ...
    shared_examples_for '[未ログイン][ない][*][無効]ベースドメイン' do
      let!(:headers) { BASE_HEADER }
      it_behaves_like 'ToError'
    end
    shared_examples_for '[未ログイン][ある][ある][無効]ベースドメイン' do
      let!(:headers) { BASE_HEADER }
      it_behaves_like 'ToError'
    end
    shared_examples_for '[未ログイン][*][*][*]存在するサブドメイン' do
    ...
    shared_examples_for '[未ログイン][ある][ない]有効なパラメータ' do
      let!(:attributes) { valid_attributes }
      it_behaves_like '[未ログイン][ある][ない][*]ベースドメイン'
      it_behaves_like '[未ログイン][*][*][*]存在するサブドメイン'
      it_behaves_like '[未ログイン][*][*][*]存在しないサブドメイン'
    end
    ...
    shared_examples_for '[未ログイン][ない][*]無効なパラメータ' do
      ...
      it_behaves_like '[未ログイン][ない][*][無効]ベースドメイン'
      it_behaves_like '[未ログイン][*][*][*]存在するサブドメイン'
      it_behaves_like '[未ログイン][*][*][*]存在しないサブドメイン'
    end
    shared_examples_for '[未ログイン][ある][ない]無効なパラメータ' do
      let!(:attributes) { invalid_attributes }
      it_behaves_like '[未ログイン][ある][ない][*]ベースドメイン'
      it_behaves_like '[未ログイン][*][*][*]存在するサブドメイン'
      it_behaves_like '[未ログイン][*][*][*]存在しないサブドメイン'
    end
    shared_examples_for '[未ログイン][ある][ある]無効なパラメータ' do
      let!(:attributes) { invalid_attributes }
      it_behaves_like '[未ログイン][ある][ある][無効]ベースドメイン'
      it_behaves_like '[未ログイン][*][*][*]存在するサブドメイン'
      it_behaves_like '[未ログイン][*][*][*]存在しないサブドメイン'
    end
    ...
    shared_examples_for '[未ログイン][ある]招待完了日時がない' do
      ...
      it_behaves_like '[未ログイン][ある][ない]無効なパラメータ'
    end
    ...
    shared_examples_for '[未ログイン][ある]招待完了日時がある' do
      ...
      it_behaves_like '[未ログイン][ある][ある]無効なパラメータ'
    end
