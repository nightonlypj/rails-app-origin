...
RSpec.describe 'Spaces', type: :request do
  ...
  describe 'GET #undo_delete' do
    ...
    shared_examples_for '[ログイン中/APIログイン中][*][ない]権限がない（なし）' do
      it_behaves_like 'ToNG(html)', 403
      it_behaves_like 'ToNG(json)', 406
    end

    shared_examples_for '[ログイン中/APIログイン中][*]削除予約がある' do |private|
    ...
    shared_examples_for '[ログイン中/APIログイン中][公開]削除予約がない' do |private|
    ...
    shared_examples_for '[ログイン中/APIログイン中][非公開]削除予約がない' do |private|
      let_it_be(:space) { FactoryBot.create(:space, private: private) }
      it_behaves_like '[ログイン中/APIログイン中][*][ない]権限がある', :admin
      it_behaves_like '[ログイン中/APIログイン中][*][ない]権限がない', :writer
      it_behaves_like '[ログイン中/APIログイン中][*][ない]権限がない', :reader
      it_behaves_like '[ログイン中/APIログイン中][*][ない]権限がない（なし）', nil
    end
    ...
    shared_examples_for '[ログイン中/APIログイン中]スペースが公開' do
      it_behaves_like '[ログイン中/APIログイン中][*]削除予約がある', false
      it_behaves_like '[ログイン中/APIログイン中][公開]削除予約がない', false
    end
    shared_examples_for '[ログイン中/APIログイン中]スペースが非公開' do
      it_behaves_like '[ログイン中/APIログイン中][*]削除予約がある', true
      it_behaves_like '[ログイン中/APIログイン中][非公開]削除予約がない', true
    end
