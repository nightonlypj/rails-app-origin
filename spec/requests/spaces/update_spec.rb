...
RSpec.describe 'Spaces', type: :request do
  ...
  describe 'PATCH /update' do
    shared_examples_for '有効なパラメータ' do
      it '更新に成功' do
        patch update_space_url, params: { space: valid_attributes }, headers: @space_headers
      ...
      it '(json)renders a ok(200) response' do
        patch update_space_url, params: valid_attributes, as: :json, headers: @space_headers.merge(json_headers)
    ...
    shared_examples_for '無効なパラメータ' do
      it '更新に失敗' do
        patch update_space_url, params: { space: invalid_attributes }, headers: @space_headers
      ...
      it '(json)renders a unprocessable(422) response' do
        patch update_space_url, params: invalid_attributes, as: :json, headers: @space_headers.merge(json_headers)
    ...
    shared_examples_for 'ベースドメイン' do
      it 'renders a not found response' do
        patch update_space_url, headers: base_headers
        expect(response).to be_not_found
      end
      it '(json)renders a not found response' do
        patch update_space_url, as: :json, headers: base_headers.merge(json_headers)
        expect(response).to be_not_found
      end
    end
    ...
    shared_examples_for '存在しないサブドメイン' do
      ...
      it 'renders a not found response' do
        patch update_space_url, headers: @space_headers
        expect(response).to be_not_found
      end
      it '(json)renders a not found response' do
        patch update_space_url, as: :json, headers: @space_headers.merge(json_headers)
        expect(response).to be_not_found
      end
    end
