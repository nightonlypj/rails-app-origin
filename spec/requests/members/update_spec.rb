...
RSpec.describe 'Members', type: :request do
  ...
  describe 'PATCH /update' do
    ...
    shared_examples_for '[ログイン中/削除予約済み][Owner][所属][自分][*]ベースドメイン' do
      ...
      it_behaves_like 'NG'
      it_behaves_like 'ToIndexNG', 'alert.member.own_update_power.owner', nil, 'alert.member.own_update_power.owner'
    end
    shared_examples_for '[ログイン中/削除予約済み][Admin][所属][自分][*]ベースドメイン' do
      let!(:headers) { BASE_HEADER }
      it_behaves_like 'NG'
      it_behaves_like 'ToIndexNG', 'alert.member.own_update_power.admin', nil, 'alert.member.own_update_power.admin'
    end
    shared_examples_for '[ログイン中/削除予約済み][Member][所属][自分/Admin/Member][*]ベースドメイン' do
      let!(:headers) { BASE_HEADER }
      it_behaves_like 'NG'
      it_behaves_like 'ToIndexNG', 'alert.member.not_update_power.admin', nil, 'alert.member.not_update_power.admin'
    end
    shared_examples_for '[ログイン中][Owner][所属][Owner][有効]ベースドメイン' do
      let!(:headers) { BASE_HEADER }
      it_behaves_like 'OK'
      it_behaves_like 'ToIndexOK', nil, 'notice.member.update', 'notice.member.update'
    end
    ...
    shared_examples_for '[ログイン中/削除予約済み][Admin/Member][所属][Owner][*]ベースドメイン' do
      let!(:headers) { BASE_HEADER }
      it_behaves_like 'NG'
      it_behaves_like 'ToIndexNG', 'alert.member.not_update_power.owner', nil, 'alert.member.not_update_power.owner'
    end
    shared_examples_for '[ログイン中][Owner/Admin][所属][Admin/Member][有効]ベースドメイン' do
      let!(:headers) { BASE_HEADER }
      it_behaves_like 'OK'
      it_behaves_like 'ToIndexOK', nil, 'notice.member.update', 'notice.member.update'
    end
    ...
    shared_examples_for '[ログイン中/削除予約済み][Admin][所属][自分]有効なパラメータ' do
      let!(:attributes) { valid_attributes }
      it_behaves_like '[ログイン中/削除予約済み][Admin][所属][自分][*]ベースドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在しないサブドメイン'
    end
    shared_examples_for '[ログイン中/削除予約済み][Member][所属][自分/Admin/Member]有効なパラメータ' do
      let!(:attributes) { valid_attributes }
      it_behaves_like '[ログイン中/削除予約済み][Member][所属][自分/Admin/Member][*]ベースドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在しないサブドメイン'
    end
    shared_examples_for '[ログイン中][Owner][所属][Owner]有効なパラメータ' do
      let!(:attributes) { valid_attributes }
      it_behaves_like '[ログイン中][Owner][所属][Owner][有効]ベースドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在しないサブドメイン'
    end
    shared_examples_for '[削除予約済み][Owner][所属][Owner]有効なパラメータ' do
      let!(:attributes) { valid_attributes }
      it_behaves_like '[削除予約済み][Owner][所属][Owner][*]ベースドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在しないサブドメイン'
    end
    shared_examples_for '[ログイン中/削除予約済み][Admin/Member][所属][Owner]有効なパラメータ' do
      let!(:attributes) { valid_attributes }
      it_behaves_like '[ログイン中/削除予約済み][Admin/Member][所属][Owner][*]ベースドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在しないサブドメイン'
    end
    shared_examples_for '[ログイン中][Owner/Admin][所属][Admin/Member]有効なパラメータ' do
      let!(:attributes) { valid_attributes }
      it_behaves_like '[ログイン中][Owner/Admin][所属][Admin/Member][有効]ベースドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在しないサブドメイン'
    end
    shared_examples_for '[削除予約済み][Owner/Admin][所属][Admin/Member]有効なパラメータ' do
      let!(:attributes) { valid_attributes }
      it_behaves_like '[削除予約済み][Owner/Admin][所属][Admin/Member][*]ベースドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在しないサブドメイン'
    end
    shared_examples_for '[未ログイン][*][未所属/存在しない][*]有効なパラメータ' do
      let!(:attributes) { valid_attributes }
      it_behaves_like '[未ログイン][*][未所属/存在しない][*][*]ベースドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在しないサブドメイン'
    end
    shared_examples_for '[ログイン中/削除予約済み][*][未所属/存在しない][自分]有効なパラメータ' do
      let!(:attributes) { valid_attributes }
      it_behaves_like '[ログイン中/削除予約済み][*][未所属/存在しない][自分][*]ベースドメイン'
      it_behaves_like '[*][*][*][自分][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][自分][*]存在しないサブドメイン'
    end
    shared_examples_for '[ログイン中/削除予約済み][*][未所属/存在しない][Owner/Admin/Member]有効なパラメータ' do
      let!(:attributes) { valid_attributes }
      it_behaves_like '[ログイン中/削除予約済み][*][未所属/存在しない][Owner/Admin/Member][*]ベースドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在しないサブドメイン'
    end
    shared_examples_for '[ログイン中/削除予約済み][Owner][所属][自分]無効なパラメータ' do
      let!(:attributes) { invalid_attributes }
      it_behaves_like '[ログイン中/削除予約済み][Owner][所属][自分][*]ベースドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在しないサブドメイン'
    end
    shared_examples_for '[ログイン中/削除予約済み][Admin][所属][自分]無効なパラメータ' do
      let!(:attributes) { invalid_attributes }
      it_behaves_like '[ログイン中/削除予約済み][Admin][所属][自分][*]ベースドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在しないサブドメイン'
    end
    shared_examples_for '[ログイン中/削除予約済み][Member][所属][自分/Admin/Member]無効なパラメータ' do
      let!(:attributes) { invalid_attributes }
      it_behaves_like '[ログイン中/削除予約済み][Member][所属][自分/Admin/Member][*]ベースドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在しないサブドメイン'
    end
    shared_examples_for '[ログイン中][Owner][所属][Owner]無効なパラメータ' do
      let!(:attributes) { invalid_attributes }
      it_behaves_like '[ログイン中][Owner][所属][Owner][無効]ベースドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在しないサブドメイン'
    end
    shared_examples_for '[削除予約済み][Owner][所属][Owner]無効なパラメータ' do
      let!(:attributes) { invalid_attributes }
      it_behaves_like '[削除予約済み][Owner][所属][Owner][*]ベースドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在しないサブドメイン'
    end
    shared_examples_for '[ログイン中/削除予約済み][Admin/Member][所属][Owner]無効なパラメータ' do
      let!(:attributes) { invalid_attributes }
      it_behaves_like '[ログイン中/削除予約済み][Admin/Member][所属][Owner][*]ベースドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在しないサブドメイン'
    end
    shared_examples_for '[ログイン中][Owner/Admin][所属][Admin/Member]無効なパラメータ' do
      let!(:attributes) { invalid_attributes }
      it_behaves_like '[ログイン中][Owner/Admin][所属][Admin/Member][無効]ベースドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在しないサブドメイン'
    end
    shared_examples_for '[削除予約済み][Owner/Admin][所属][Admin/Member]無効なパラメータ' do
      let!(:attributes) { invalid_attributes }
      it_behaves_like '[削除予約済み][Owner/Admin][所属][Admin/Member][*]ベースドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在しないサブドメイン'
    end
    shared_examples_for '[未ログイン][*][未所属/存在しない][*]無効なパラメータ' do
      let!(:attributes) { invalid_attributes }
      it_behaves_like '[未ログイン][*][未所属/存在しない][*][*]ベースドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在しないサブドメイン'
    end
    shared_examples_for '[ログイン中/削除予約済み][*][未所属/存在しない][自分]無効なパラメータ' do
      let!(:attributes) { invalid_attributes }
      it_behaves_like '[ログイン中/削除予約済み][*][未所属/存在しない][自分][*]ベースドメイン'
      it_behaves_like '[*][*][*][自分][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][自分][*]存在しないサブドメイン'
    end
    shared_examples_for '[ログイン中/削除予約済み][*][未所属/存在しない][Owner/Admin/Member]無効なパラメータ' do
      let!(:attributes) { invalid_attributes }
      it_behaves_like '[ログイン中/削除予約済み][*][未所属/存在しない][Owner/Admin/Member][*]ベースドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在するサブドメイン'
      it_behaves_like '[*][*][*][Owner/Admin/Member][*]存在しないサブドメイン'
    ...
    shared_examples_for '[ログイン中/削除予約済み][Owner][所属]対象が自分' do
      ...
      it_behaves_like '[ログイン中/削除予約済み][Owner][所属][自分]有効なパラメータ'
      it_behaves_like '[ログイン中/削除予約済み][Owner][所属][自分]無効なパラメータ'
    end
    shared_examples_for '[ログイン中/削除予約済み][Admin][所属]対象が自分' do
      let!(:user_code) { user.code }
      let!(:target_member) { member }
      it_behaves_like '[ログイン中/削除予約済み][Admin][所属][自分]有効なパラメータ'
      it_behaves_like '[ログイン中/削除予約済み][Admin][所属][自分]無効なパラメータ'
    end
    shared_examples_for '[ログイン中/削除予約済み][Member][所属]対象が自分' do
      let!(:user_code) { user.code }
      let!(:target_member) { member }
      it_behaves_like '[ログイン中/削除予約済み][Member][所属][自分/Admin/Member]有効なパラメータ'
      it_behaves_like '[ログイン中/削除予約済み][Member][所属][自分/Admin/Member]無効なパラメータ'
    end
    shared_examples_for '[ログイン中/削除予約済み][*][未所属/存在しない]対象が自分' do
      let!(:user_code) { user.code }
      let!(:target_member) { nil } # Tips: 権限がない為、紐付かない
      it_behaves_like '[ログイン中/削除予約済み][*][未所属/存在しない][自分]有効なパラメータ'
      it_behaves_like '[ログイン中/削除予約済み][*][未所属/存在しない][自分]無効なパラメータ'
    end
    shared_examples_for '[ログイン中][Owner][所属]対象がOwner' do
      let!(:user_code) { @create_users[0].code }
      let!(:target_member) { @create_members[0] }
      it_behaves_like '[ログイン中][Owner][所属][Owner]有効なパラメータ'
      it_behaves_like '[ログイン中][Owner][所属][Owner]無効なパラメータ'
    end
    shared_examples_for '[削除予約済み][Owner][所属]対象がOwner' do
      let!(:user_code) { @create_users[0].code }
      let!(:target_member) { @create_members[0] }
      it_behaves_like '[削除予約済み][Owner][所属][Owner]有効なパラメータ'
      it_behaves_like '[削除予約済み][Owner][所属][Owner]無効なパラメータ'
    end
    shared_examples_for '[ログイン中/削除予約済み][Admin/Member][所属]対象がOwner' do
      let!(:user_code) { @create_users[0].code }
      let!(:target_member) { @create_members[0] }
      it_behaves_like '[ログイン中/削除予約済み][Admin/Member][所属][Owner]有効なパラメータ'
      it_behaves_like '[ログイン中/削除予約済み][Admin/Member][所属][Owner]無効なパラメータ'
    end
    shared_examples_for '[ログイン中][Owner/Admin][所属]対象がAdmin' do
      let!(:user_code) { @create_users[1].code }
      let!(:target_member) { @create_members[1] }
      it_behaves_like '[ログイン中][Owner/Admin][所属][Admin/Member]有効なパラメータ'
      it_behaves_like '[ログイン中][Owner/Admin][所属][Admin/Member]無効なパラメータ'
    end
    shared_examples_for '[削除予約済み][Owner/Admin][所属]対象がAdmin' do
      let!(:user_code) { @create_users[1].code }
      let!(:target_member) { @create_members[1] }
      it_behaves_like '[削除予約済み][Owner/Admin][所属][Admin/Member]有効なパラメータ'
      it_behaves_like '[削除予約済み][Owner/Admin][所属][Admin/Member]無効なパラメータ'
    end
    ...
    shared_examples_for '[未ログイン][*][未所属/存在しない]対象がOwner' do
      ...
      it_behaves_like '[未ログイン][*][未所属/存在しない][*]有効なパラメータ'
      it_behaves_like '[未ログイン][*][未所属/存在しない][*]無効なパラメータ'
    end
    shared_examples_for '[ログイン中/削除予約済み][*][未所属/存在しない]対象がOwner' do
      let!(:user_code) { @create_outside_users[0].code }
      let!(:target_member) { @create_outside_members[0] }
      it_behaves_like '[ログイン中/削除予約済み][*][未所属/存在しない][Owner/Admin/Member]有効なパラメータ'
      it_behaves_like '[ログイン中/削除予約済み][*][未所属/存在しない][Owner/Admin/Member]無効なパラメータ'
    end
