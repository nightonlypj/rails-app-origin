...
RSpec.describe 'CustomerUsers', type: :request do
  ...
  describe 'PATCH /destroy' do
    ...
    shared_examples_for 'OK' do
      it 'メンバーが削除される' do
        expect do
          delete customer_user_path(customer_code: customer_code, user_code: user_code), headers: headers
        end.to change(CustomerUser, :count).by(-1)
      end
      it '(json)メンバーが削除される' do
        expect do
          delete customer_user_path(customer_code: customer_code, user_code: user_code, format: :json), headers: headers
        end.to change(CustomerUser, :count).by(-1)
      end
    end
    ...
    shared_examples_for '[ログイン中/削除予約済み][Owner権限][所属顧客][対象自分]ベースドメイン' do
      ...
      it_behaves_like 'NG'
      it_behaves_like 'ToIndexNG', 'alert.customer_user.own_destroy_power.owner', nil, 'alert.customer_user.own_destroy_power.owner'
    end
    shared_examples_for '[ログイン中/削除予約済み][Admin権限][所属顧客][対象自分]ベースドメイン' do
      let!(:headers) { BASE_HEADER }
      it_behaves_like 'NG'
      it_behaves_like 'ToIndexNG', 'alert.customer_user.own_destroy_power.admin', nil, 'alert.customer_user.own_destroy_power.admin'
    end
    shared_examples_for '[ログイン中/削除予約済み][Member権限][所属顧客][対象自分/Admin/Member]ベースドメイン' do
      let!(:headers) { BASE_HEADER }
      it_behaves_like 'NG'
      it_behaves_like 'ToIndexNG', 'alert.customer_user.not_destroy_power.admin', nil, 'alert.customer_user.not_destroy_power.admin'
    end
    shared_examples_for '[ログイン中][Owner権限][所属顧客][対象Owner]ベースドメイン' do
      let!(:headers) { BASE_HEADER }
      it_behaves_like 'OK'
      it_behaves_like 'ToIndexOK', nil, 'notice.customer_user.destroy', 'notice.customer_user.destroy'
    end
    ...
    shared_examples_for '[ログイン中/削除予約済み][Admin権限/Member][所属顧客][対象Owner]ベースドメイン' do
      let!(:headers) { BASE_HEADER }
      it_behaves_like 'NG'
      it_behaves_like 'ToIndexNG', 'alert.customer_user.not_destroy_power.owner', nil, 'alert.customer_user.not_destroy_power.owner'
    end
