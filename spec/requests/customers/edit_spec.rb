...
RSpec.describe 'Customers', type: :request do
  ...
  describe 'GET #edit' do
    ...
    shared_examples_for 'ToOK' do
      it '成功ステータス' do
        get edit_customer_path(customer_code: customer_code), headers: headers
    ...
    shared_examples_for 'ToBase' do |alert, notice|
      it 'ベースドメインにリダイレクト' do
        get edit_customer_path(customer_code: customer_code), headers: headers
        expect(response).to redirect_to("//#{Settings['base_domain']}#{edit_customer_path(customer_code: customer_code)}")
        expect(flash[:alert]).to alert.present? ? eq(I18n.t(alert)) : be_nil
        expect(flash[:notice]).to notice.present? ? eq(I18n.t(notice)) : be_nil
      end
    end
    ...
    # テストケース
    shared_examples_for '[ログイン中][Owner][所属]ベースドメイン' do
    ...
    shared_examples_for '[ログイン中/削除予約済み][Admin/Member/ない][所属]ベースドメイン' do
      let!(:headers) { BASE_HEADER }
      it_behaves_like 'ToTop', 'alert.customer.not_update_power', nil
    end
    ...
    shared_examples_for '[ログイン中][Owner]顧客に所属' do
      ...
      it_behaves_like '[ログイン中][Owner][所属]ベースドメイン'
