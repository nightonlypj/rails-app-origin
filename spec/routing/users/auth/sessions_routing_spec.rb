...
RSpec.describe Users::Auth::SessionsController, type: :routing do
  describe 'routing' do
    ...
    it 'routes to #create' do
      expect(post: '/users/auth/sign_in').to route_to('users/auth/sessions#create', format: :json)
      expect(post: '/users/auth/sign_in.json').to route_to('users/auth/sessions#create', format: 'json')
    end
    it 'routes to #destroy' do
      expect(delete: '/users/auth/sign_out').to route_to('users/auth/sessions#destroy', format: :json)
