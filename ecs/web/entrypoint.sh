...
if [ "$NGINX_WORKER_RLIMIT_NOFILE" = '' ]; then
    export NGINX_WORKER_RLIMIT_NOFILE=32768 # 最大値: 524288(cat /proc/sys/fs/file-max) / WORKER_PROCESSES
...
if [ "$NGINX_WORKER_CONNECTIONS" = '' ]; then
    export NGINX_WORKER_CONNECTIONS=8192 # 最大値: WORKER_RLIMIT_NOFILE / 4
